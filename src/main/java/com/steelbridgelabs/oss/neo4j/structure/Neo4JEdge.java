...
public class Neo4JEdge extends Neo4JElement implements Edge {
    ...
    public static  Neo4JEdge createEdge(Neo4JGraph graph, Relationship relationship) {

        Iterator<Vertex> vertices = graph.currentSession().vertices(new Object[]{relationship.startNodeId(), relationship.endNodeId()});

        Neo4JVertex out = (Neo4JVertex) vertices.next();
        Neo4JVertex in = (Neo4JVertex) vertices.next();

        return new Neo4JEdge(graph, graph.currentSession(), new Neo4JNativeElementIdProvider(), out, relationship,  in);
    }
